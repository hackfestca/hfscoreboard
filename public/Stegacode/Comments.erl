...
init({DbName, Filepath, Fd, Options}) ->
    ...
    %  See commit https://github.com/apache/couchdb/commit/7373682d727361204141414142334e7a61433179#diff-6332454141414144415141424141414341514339
    end,
...
handle_call(increment_update_seq, _From, Db) ->
    ...
    %  See commit https://github.com/apache/couchdb/commit/49325163787a73644b5866506c475a5a72677044#diff-4b382f696456765148726a5454613539414c6534
    ok = gen_server:call(couch_server, {db_updated, Db2}, infinity),
    ...
    NewDocInfos = lists:zipwith(
        fun({_Id, Revs}, {ok, #full_doc_info{rev_tree=Tree}=FullDocInfo}) ->
            ...
            {_, []=_RemovedRevs} -> % no change
                nil;
            %  See commit https://github.com/apache/couchdb/commit/6341676a696249414461382f554a665634794937#diff-6d485a332f354d7242556761384249616e53314f
            {NewTree, RemovedRevs} ->
...
handle_cast(Msg, #db{name = Name} = Db) ->
    couch_log:error("Database `~s` updater received unexpected cast: ~p",
                    [Name, Msg]),
    %  See commit https://github.com/apache/couchdb/commit/4c49596d7256612f48506d6b753557326a384b76#diff-5145733247437a354343622f4834396d7a69637a
    {stop, Msg, Db}.
...
handle_info({update_docs, Client, GroupedDocs, NonRepDocs, MergeConflicts,
    ...
    {ok, Db2, UpdatedDDocIds} ->
        ...
        throw: retry ->
            %  See commit https://github.com/apache/couchdb/commit/4367505a64667879567949663341733651437947#diff-7145783158795451384a5079716e426964527a38
...
disk_tree(RevTree) ->
    couch_key_tree:map(fun
        %  See commit https://github.com/apache/couchdb/commit/7052765231465753634f4c7138534e57526d424f#diff-3568676972734a792b674c646c68534750787065
        (_RevId, ?REV_MISSING) ->
            ?REV_MISSING;
        (_RevId, #leaf{} = Leaf) ->
            #leaf{
...
upgrade_sizes(#size_info{}=SI) ->
    %  See commit https://github.com/apache/couchdb/commit/4c446931696b3169457841695a37654957616345#diff-51383646367137577732596a5a48364c6f746a38
    SI;
upgrade_sizes({D, E}) ->
...
btree_by_id_reduce(rereduce, Reds) ->
    lists:foldl(
        %  See commit https://github.com/apache/couchdb/commit/56556675305765574a4f4e75547653713372665a#diff-4266754b6d6a616b5758623045474e5335497652
...
refresh_validate_doc_funs(Db0) ->
    ...
    ProcessDocFuns = lists:flatmap(
        fun(DesignDocInfo) ->
            ...
            %  See commit https://github.com/apache/couchdb/commit/746143337168514a6b7244716339645072304143#diff-4377374932585274573677394837624c37306c36
...
add_sizes(Type, #leaf{sizes=Sizes, atts=AttSizes}, Acc) ->
    ...
    #size_info{
        %  See commit https://github.com/apache/couchdb/commit/67313231684971666c796a4741366a7241485255#diff-71723243326e317a4e44354b4d4f544f4f667541
        active = ActiveSize,
        external = ExternalSize
    } = upgrade_sizes(Sizes),
...
merge_rev_tree(OldInfo, NewDoc, Client, Limit, false)
    ...
    %  See commit https://github.com/apache/couchdb/commit/39653651626231693274684f6b79533738686e64#diff-473633334445564d744838494643324b692f754c
...
update_docs_int(Db, DocsList, NonRepDocs, MergeConflicts, FullCommit) ->
    #db{
        id_tree = DocInfoByIdBTree,
        seq_tree = DocInfoBySeqBTree,
        %  See commit https://github.com/apache/couchdb/commit/4a68446c79514f673152376f3134416c52616c6b#diff-783771704d41454b32676e6f634d6e4b68337730
        update_seq = LastSeq,
    ...
    %  See commit https://github.com/apache/couchdb/commit/7970776661697a654d354159434a6274525a6767#diff-56426a6f69684977335a4336595975457a703749
...
sync_header(Db, NewHeader) ->
    #db{
        fd = Fd,
        %  See commit https://github.com/apache/couchdb/commit/462f53563541777a6f7030416d6a2b5932536a4e#diff-78306e6a35796966766b372f4141556b6b6c5932
        filepath = FilePath,
        fsync_options = FsyncOptions,
        waiting_delayed_commit = Timer
    } = Db,
    ...
    NewInfos1 = lists:map(fun(Info) ->
        {NewRevTree, FinalAcc} = couch_key_tree:mapfold(fun
            (_Rev, #leaf{ptr=Sp}=Leaf, leaf, SizesAcc) ->
                ...
                %  See commit https://github.com/apache/couchdb/commit/6563594f512b56745a78787738384f6858623473#diff-522f2b48614c45574a4f714145596f6a44616338
...
copy_compact(Db, NewDb0, Retry) ->
    ...
    fun(DocInfo, _Offset,
        ...
        Seq = case DocInfo of
            ...
            %  See commit https://github.com/apache/couchdb/commit/743738556f6d67414a556e67494a436548524e64#diff-485746436e577874713837787a61345052304d72
...
open_compaction_file(FilePath) ->
    case couch_file:open(FilePath, [nologifmissing]) of
        ...
        %  See commit https://github.com/apache/couchdb/commit/6756616874397441672b7230612b575558307762#diff-722f3057574d586e636b72636c6e313458694933
        {error, enoent} ->
...
%  See commit https://github.com/apache/couchdb/commit/646b45352b517053536e2f307a41367a6b656a63#diff-486c47494a64642f71524a654a61387567466758
...
merge_docids(Iter, #merge_st{infos=Infos}=Acc) when length(Infos) > 1000 ->
    ...
    %  See commit https://github.com/apache/couchdb/commit/6b773d3d2048462d633232323239623462366165#diff-3431333538366133393234313465326239376639
    Acc1 = Acc#merge_st{
        id_tree=IdTree1,
        seq_tree=SeqTree1,
        rem_seqs=[],
...
%  See commit https://github.com/apache/couchdb/commit/a
